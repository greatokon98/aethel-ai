window.AdminAPI = (function() {
  ...
  async function githubFetch(method, path, body) {
    ...
    if (path.startsWith('/repos/')) url = 'https://api.github.com' + path;
  ...
  async function getFile(path) {
    try {
      var data = await githubFetch('GET', '/contents/' + path + '?ref=' + CONFIG.branch);
  ...
  async function saveFile(path, content, sha) {
    ...
    var payload = { message: 'Update ' + path, content: encoded, branch: CONFIG.branch };
    if (sha) payload.sha = sha;
    var data = await githubFetch('PUT', '/contents/' + path, payload);
    return data.content.sha;
  }

  async function deleteFile(path, sha) {
    await githubFetch('DELETE', '/contents/' + path, { message: 'Delete ' + path, sha: sha, branch: CONFIG.branch });
  }
  ...
  async function listPosts() {
    var data;
    try {
      data = await githubFetch('GET', '/contents/src/content/posts?ref=' + CONFIG.branch);
    } catch(e) {
    ...
    for (var i = 0; i < files.length; i++) {
      var f = files[i];
      try {
        var content = await githubFetch('GET', '/contents/src/content/posts/' + f.name);
  ...
  async function savePost(slug, attrs, body) {
    ...
    return saveFile(path, content, sha);
  }
  ...
  async function loadSettings() {
    try {
      var file = await getFile('src/config/site.json');
  ...
  async function saveSettings(settings) {
    ...
    var sha = settings._sha || null;
    var newSha = await saveFile('src/config/site.json', content, sha);
    return newSha;
  }

  // --- Messages ---
  var MESSAGES_PATH = 'data/messages.json';

  async function loadMessages() {
    try {
      var file = await getFile(MESSAGES_PATH);
      if (!file) return [];
      var data = JSON.parse(file.content);
      data._sha = file.sha;
      return data;
    } catch(e) {
      return { messages: [], _sha: null };
    }
  }

  async function saveMessages(data) {
    var content = JSON.stringify({ messages: data.messages }, null, 2);
    var sha = data._sha || null;
    var newSha = await saveFile(MESSAGES_PATH, content, sha);
    return newSha;
  }

  // --- Search Queries ---
  var SEARCH_PATH = 'data/search-queries.json';

  async function loadSearchQueries() {
    try {
      var file = await getFile(SEARCH_PATH);
  ...
  async function saveSearchQueries(data) {
    ...
    var sha = data._sha || null;
    var newSha = await saveFile(SEARCH_PATH, content, sha);
    return newSha;
  }

  // --- Automation Runs ---
  var AUTOMATION_PATH = 'data/automation-runs.json';

  async function loadAutomationRuns() {
    try {
      var file = await getFile(AUTOMATION_PATH);
  ...
  async function saveAutomationRuns(data) {
    ...
    var sha = data._sha || null;
    var newSha = await saveFile(AUTOMATION_PATH, content, sha);
    return newSha;
  }
  ...
  return {
    ...
    // Search
    loadSearchQueries: loadSearchQueries,
